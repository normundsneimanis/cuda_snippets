...
class NBodyDemo
{
    public:
        ...
        static void init(int numBodies, int numDevices, int blockSize,
                         bool usePBO, bool useHostMem, bool useP2P, bool useCpu)
        {
            m_singleton->_init(numBodies, numDevices, blockSize, usePBO, useHostMem, useP2P, useCpu);
        }
    ...
    private:
        ...
        void _init(int numBodies, int numDevices, int blockSize,
                   bool bUsePBO, bool useHostMem, bool useP2P, bool useCpu)
        {
            if (useCpu)
            {
            ...
            {
                m_nbodyCuda = new BodySystemCUDA<T>(numBodies, numDevices, blockSize, bUsePBO, useHostMem, useP2P);
                m_nbody = m_nbodyCuda;
                m_nbodyCpu = 0;
            }
...
main(int argc, char **argv)
{
    ...
    printf("*** EuroCC RTU CUDA training course 2021. ***\n\n");
    ...
    NBodyDemo<float>::init(numBodies, numDevsRequested, blockSize, !(benchmark || compareToCPU || useHostMem), useHostMem, useP2P, useCpu);
    NBodyDemo<float>::reset(numBodies, NBODY_CONFIG_SHELL);

    if (bSupportDouble)
    {
        NBodyDemo<double>::Create();
        NBodyDemo<double>::init(numBodies, numDevsRequested, blockSize, !(benchmark || compareToCPU || useHostMem), useHostMem, useP2P, useCpu);
        NBodyDemo<double>::reset(numBodies, NBODY_CONFIG_SHELL);
    }
