...
BitmapImage::BitmapImage(const BitmapImage& input, bool copyData) : _rawData(NULL), _paletteColors(NULL), _dataSize(input._dataSize), 
	_bitDepth(input._bitDepth), _grayScale(input._grayScale) {
	...
	if (copyData) {
		memcpy(_rawData, input._rawData, _dataSize);
	}
}
...
bool BitmapImage::saveToFile(const std::string& fileName) {
	...
	double actualBytesPerRow = bytesPerRow + bytePaddingPerRow;

	double totalPixelBytes = _height * actualBytesPerRow;
...
bool BitmapImage::write32bitRow(BYTE* buffer, int bufferSize, int row) { 
	...
	memcpy(buffer, &_rawData[row * _width * 4], _width * 4); 
...
bool BitmapImage::read8bitRow(BYTE* buffer, int bufferSize, int row, bool grayScale) {
	...
	if (grayScale) {
		memcpy(&_rawData[offset], buffer, _width); 
	} else {
		for (int x = 0; x < _width; x++) {
			int index = buffer[x];
			memcpy(&_rawData[4 * (x + offset)], &_paletteColors[index], 4); 
		}
	}
...
bool BitmapImage::read32bitRow(BYTE* buffer, int bufferSize, int row) { 
	if (_width * 4 > bufferSize) { 
		return false; 
	}

	memcpy(&_rawData[row * _width * 4], buffer, _width * 4);
...
void BitmapImage::generateSaltAndPepperNoise() {
	...
	srand((unsigned int)time(NULL)); // time(NULL) is used as SEED for random number generator
